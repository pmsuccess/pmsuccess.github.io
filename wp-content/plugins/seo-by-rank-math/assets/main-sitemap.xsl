			...
			<head>
				<xsl:choose>
					<xsl:when test="kml:kml">
						<title>Locations Sitemap - Roger 老師｜周龍鴻的Blog</title>
					</xsl:when>
					<xsl:otherwise>
						<title>XML Sitemap - Roger 老師｜周龍鴻的Blog</title>
					</xsl:otherwise>
				</xsl:choose>
				...
				<xsl:choose>
					<xsl:when test="kml:kml">
						...
						<div id="content">
							...
							<p class="expl">
								<a href="https://rogersnotes.com/sitemap_index.xml">&#8592; Sitemap Index</a>							</p>
